...
- 🎭 **`/role`** — список готовых ролей (**Мотиватор**, **Браток**, **Стартапер**, **Друн**) и подсказка, как задать свою.
...
Пользователь: /role
Бот: Выбери роль кнопкой ниже 👇 или опиши свою текстом.
     [Мотиватор] [Браток]
     [Стартапер] [Друн]

Пользователь: Браток
Бот: Готово! Теперь я — «Браток». История диалога очищена ...

Пользователь: с чего начать утро?
Бот: Слышь сюда, братишка: подъём, водички дёрнул, зарядку — и погнали...

Пользователь: а спорт обязательно?
Бот: (помнит контекст) Зуб даю, брат, без движения никак ...
```
...
│   ├── config/
│   │   └── config.go                    # чтение и валидация конфигурации из env (+ .env)
│   ├── domain/
...
├── pkg/
│   └── utils/
│       └── markdown.go                  # утилита: Markdown → HTML Telegram
...
| `OPENROUTER_IGNORE_PROVIDERS` | ❌ | —                                             | Провайдеры, которые не использовать (через запятую). Напр. `Azure` (если в privacy-настройках разрешены другие) |
| `OPENROUTER_PROVIDER_ORDER` | ❌ | —                                             | Приоритетный порядок провайдеров (через запятую)  |
| `OPENROUTER_TEMPERATURE` | ❌ | `0.8`                                            | «Температура» генерации (выше = живее/креативнее) |
